  ...
  return (
    ...
    <header className="sticky top-0 z-50 w-full glass-panel border-b border-gray-100">
      <div className="max-w-7xl mx-auto px-2">
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden absolute top-full left-0 w-full bg-white border-b border-gray-100 shadow-lg pb-4 max-h-[80vh] overflow-y-auto">
            <div className="flex flex-col px-4 pt-2 pb-4 space-y-1">
              ...
              <div className="px-3 py-2">
                <div className="text-base font-medium text-gray-900 mb-2">Shop By Category</div>
                <div className="flex flex-col space-y-1 pl-4 border-l-2 border-gray-100 ml-2">
                  <Link to="/shop" className="py-2 text-sm text-gray-600 hover:text-primary font-medium" onClick={() => setIsMobileMenuOpen(false)}>
                    All Products
                  </Link>
                  {categories.filter(c => c !== 'All').map(category => (
                    <Link 
                      key={category} 
                      to={`/shop?category=${encodeURIComponent(category)}`} 
                      className="py-2 flex items-center text-sm text-gray-600 hover:text-primary transition-colors font-medium"
